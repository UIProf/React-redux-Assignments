...
describe("todos reducer", () => {
  ...
  describe("addTodo", () => {
    ...
    it("should remove space on new todo", () => {
      const state = applyActionsFrom([
        {id: 1, title: "Item A", completed: false},
        {id: 3, title: "Item B", completed: true},
        {id: 7, title: "Item C", completed: false},
      ], actions.addTodo("Buy some milk     "));
      const lengthofstring = state[3].title;
      expect(lengthofstring).to.equal("Buy some milk");
    });
  });
  ...
  describe("editTodo", () => {
    ...
    it("remove white space with updates the title of the relevant todo", () => {
      const state = applyActionsFrom(initial, actions.editTodo(3, "Renamed      "));
      assert.equal(state[1].title.trim(), "Renamed");
    });
  });
  ...
  describe("clearCompleted", () => {
    const initial = [
      ...
      {id: 3, title: "Item B", completed: true},
      {id: 7, title: "Item C", completed: true},
    ];
    it("should all clear completed !", () =>{
        const state = applyActionsFrom(initial, actions.clearCompleted(false));
        assert.deepEqual(state.map(t => t.completed), [false, false, false]);
    });
